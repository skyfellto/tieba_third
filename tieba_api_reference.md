...
我的需求是修改用户详情页，使其能显示用户详细信息与用户所发的帖子与回帖和关注的吧。用户详情页整体采用**垂直流式布局**，视觉层次清晰，分为**顶部导航栏、用户信息头部、标签栏、内容切换 Tab 栏、Tab 内容区**5 个核心模块，整体配色要符合白昼和夜间的主题。顶部导航栏与下方用户信息区的背景完全融合，延伸至状态栏下方，右侧有三个点的更多操作图标。用户信息头部区域整体为**左图右文的水平布局**，垂直居中对齐；左侧为头像区，大尺寸圆形头像，头像加载失败时显示灰色占位；右侧为用户信息区（垂直排列），第一行为主用户名，第二行为副用户名，第三行为4 个统计项，水平均分排列，每个项为**数字在上、文字在下**的垂直布局，项与项之间用细浅灰色竖线分隔，统计项分别为关注，粉丝，获赞，吧龄；个性签名位于统计项下方，左对齐，无内容时显示 “这个人很懒，什么都没有写”；用户标签栏位于个性签名下方，左对齐水平排列，三个标签分别为性别标签（圆角矩形背景，内部性别符号（♂/♀）），ID 标签（圆角矩形背景，内部 ID 号），IP 属地标签（圆角矩形背景，内部白色 “来自：XX” 文字）；**内容切换 Tab 栏**包含 2 个 Tab，水平均分排列：帖子、关注吧，所有文字垂直居中，区别选中和未选中状态；Tab 内容区的帖子部分样式同动态页面的帖子样式，点击帖子能够进入该帖子详情页，关注吧部分同贴吧页面的关注的吧部分，但是不显示吧等级与是否签到，点击贴吧能够进入贴吧详细页，tab内容区的滑动效果可以使用懒加载；用户在tab内容区下滑时顶部导航栏与用户信息头部有动画效果，动画效果完全同贴吧详情页的动画效果，只不过是除了用户头像与用户名都逐渐消失，用户名与用户头像逐渐水平布局，最后内容切换tab栏吸附在顶部导航栏下方。





我的需求是点击动态页面，贴吧详情页的帖子部分的用户头像与用户名，帖子详情页，回复详情页的用户头像与用户名能够跳转到对应的用户详情页，样式完全同从我的页面进入的用户详情页；并在本地存储记录访问过的用户，在浏览记录页面的访问用户部分能显示出来，并且点击能够进入相应的用户详情页，访问用户部分的样式同经过贴吧部分，不过是贴吧头像换成用户头像，贴吧名换成用户名，仍然有日期进行隔断





我的需求是在进入用户详情页时的加载过程中，优先加载所选择的用户的圆形头像、主用户名文字，而不是登录用户的（除了进入登录用户的详情页时），和最终布局位置完全一致，副用户名区域用**长条形圆角矩形**占位，统计项区域用四个统计项位置用**等宽的圆角矩形**占位，占位符之间保留原有的浅灰色竖线分隔，个性签名区域用**长条形圆角矩形**占位，左对齐，宽度和实际签名行一致，标签栏区域用**三个短条形圆角矩形**占位，间距和实际标签间距完全匹配，颜色匹配不同的主题。在浏览记录页面点击顶部右侧的更多图标时出现使用**`dropdown_button2`**（已添加）的下拉框，下拉框中可以选择删除与倒序；点击删除后将顶部的返回键替换为“全选”按钮，“浏览记录”字样替换为“删除”字样，更多图标替换为删除图标，tab栏所选中的内容部分的头像前方显示选择框，头像，标题，预览文字/吧名/用户名全部右移，文字的缩略长度更短，使其有更好的视觉效果，点击全选按钮后将当前tab栏所选中的内容的选择框全部打钩，再点击一次选择框则取消所有选项框，点击删除图标后则删除选择框打钩的记录并退出删除功能；点击倒序后则将tab栏的所有记录按与现在时间倒序过来显示，再次点击则又倒序过来，但是不记录用户所选择的倒序排序，一直默认按现在的时间顺序显示
...
我的需求是在搜索页面触发搜索请求后能够跳转到搜索结果页。搜索结果页顶部样式同搜索页面的顶部，并且在下方增加tab栏，分别显示“贴吧”“帖子”“用户”，并且能够左右切换，进入搜索结果页默认是在“贴吧”部分。搜索结果页的搜索结果部分根据tab栏的所在部分发起请求：若是在“贴吧”部分则发起贴吧的搜索请求，在得到响应数据后进行数据的渲染，贴吧单项整体是垂直布局，若响应结果含有exactMatch，则在最上方右侧显示“推荐”，下方贴吧信息部分，整体水平布局，左侧是贴吧头像，右侧是垂直布局，第一行是贴吧名，第二行是贴吧简介，第三行是贴吧信息，样式为“关注：{关注数}  帖子：{帖子数}",若响应结果含有fuzzyMatch，在最上方右侧显示“相关吧”，下方贴吧信息部分，整体水平布局，左侧是贴吧头像，右侧是垂直布局，第一行是贴吧名，第二行是贴吧信息，样式为“关注：{关注数}  帖子：{帖子数}",优先展示”推荐“部分； 若是在”帖子“部分则发起帖子的搜索请求，在得到响应数据后进行数据的渲染，整体样式同动态页面的帖子部分，并且也能够触底加载； 若是在”用户“部分，则发起用户的搜索请求，在得到响应数据后进行数据的渲染，若响应结果含有exactMatch，则在最上方右侧显示“推荐”，下方用户信息部分，整体水平布局，左侧是用户头像，右侧是用户名，优先展示”show_nickname“，旁边是”（user_nickname）“，若只有user_nickname则只显示user_nickname，若响应结果含有fuzzyMatch则在最上方右侧显示“相关用户”，下方用户信息部分，整体水平布局，左侧是用户头像，右侧是用户名，优先展示”show_nickname“，旁边是”（user_nickname）“，若只有user_nickname则只显示user_nickname，   相关请求接口： 贴吧搜索（https://tieba.baidu.com/mo/q/search/forum?word=搜索框内容），帖子搜索（https://tieba.baidu.com/mo/q/search/thread?word=搜索框内容&pn=页数&st=5&tt=1&ct=1&is_use_zonghe=1&cv=99.9.101），用户搜索（https://tieba.baidu.com/mo/q/search/user?word=搜索框内容）
